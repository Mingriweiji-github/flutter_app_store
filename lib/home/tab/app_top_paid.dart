...
class AppTopPaid extends StatelessWidget {
  final TopFeed paidFeed;
  const AppTopPaid({ Key? key,required this.paidFeed }) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return this.paidFeed.results == null || this.paidFeed.results?.length == 0
        ? Text('占位')
        : ListView.builder(
            itemCount: this.paidFeed.results?.length,
            itemBuilder: (context, index) {
              return _buildRow(
                  index + 1, this.paidFeed.results![index], context);
            });
  }
